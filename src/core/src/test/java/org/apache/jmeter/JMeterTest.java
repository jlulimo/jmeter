...
class JMeterTest extends JMeterTestCase implements JMeterSerialTest {
    private static Logger logger = LoggerFactory.getLogger(JMeterTest.class);
    ...
    void testFailureWhenJmxDoesNotExist() {
        ...
        try {
            ...
            Assertions.assertTrue(e.getMessage().contains("doesn't exist or can't be opened"), "When the file doesn't exist, this method 'runNonGui' should have a detailed message");
        }
    }
    ...
    void testSuccessWhenJmxExists() throws IOException, ConfigurationException {
        ...
        try (FileOutputStream os = new FileOutputStream(temp); Writer fw = new OutputStreamWriter(os, StandardCharsets.UTF_8); BufferedWriter out = new BufferedWriter(fw)) {
            out.write(testPlan);
        }
    ...
    void testFailureWithMissingPlugin() throws IOException {
        ...
        try {
            ...
            Assertions.assertTrue(e.getMessage().contains("Error in NonGUIDriver Problem loading XML from"), "When the plugin doesn't exist, the method 'runNonGui' should have a detailed message");
        } finally {
            Assertions.assertTrue(temp.delete(), () -> "File " + temp.getAbsolutePath() + " should have been deleted");
        }
    }
    ...
    @Test
    public void testGenerteReportFromJtl() {
        JMeter jMeter = new JMeter();
        try {
            String[] args = {"-g", "/Users/limo/Desktop/result/abc", "-o", "/Users/limo/Desktop/report/"};
            jMeter.start(args);
        } catch (Exception e) {
